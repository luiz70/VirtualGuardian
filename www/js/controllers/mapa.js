...
.controller('Mapa', function($scope,Mapa,uiGmapIsReady,$ionicHistory,$rootScope,$timeout,socket,$ionicScrollDelegate,$animate,Lugar,uiGmapGoogleMapApi,$window,Ubicacion){//,Evento,Message) {
	...
	$scope.$on('$ionicView.enter',function(){
		...
		try{
			...
			if($rootScope.reloadMapa){
			if($rootScope.ubicacion)$rootScope.map.center={latitude:$rootScope.ubicacion.position.latitude,longitude:$rootScope.ubicacion.position.longitude}
  ...
  $scope.abreLugar=function(lugar){
	  ...
	  if($rootScope.radio.activo){
	  	Ubicacion.stopPosition();
		$rootScope.ubicacion.position={latitude: lugar.geometry.location.lat(), longitude:  lugar.geometry.location.lng()}
	  }
	...
	$scope.viajaUber=function(data){
		var uberData = {
			...
			fromLatitude: ""+$rootScope.ubicacion.location.latitude,
			fromLongitude: ""+$rootScope.ubicacion.location.longitude,
			//fromNickname: "UberHQ",

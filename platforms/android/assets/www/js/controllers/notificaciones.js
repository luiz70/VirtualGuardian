...
.controller('Notificaciones', function($scope,$timeout,ionicMaterialMotion,ionicMaterialInk,socket,$rootScope,Message,Notificacion,$ionicScrollDelegate,$state,$ionicViewSwitcher,Llamada,Ubicacion) {
	...
	$scope.$on('$ionicView.beforeEnter',function(){
		$rootScope.push.notificaciones=0;
	})
	...
	if($rootScope.notificaciones && $rootScope.notificaciones.length>0){
		$timeout(function(){
		$scope.animate();
		},300)
	...
	$scope.abreNotificacion=function(data){
		...
		if(data.Tipo==8){
			//TIPS VIRTUAL
			$rootScope.abreTip(data.IdNotificacion)
		}else{
		var buttons=[];

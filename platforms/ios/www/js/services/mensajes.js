...
.factory('Message', function(localStorageService,$ionicLoading,$ionicPopup,$cordovaToast,$ionicActionSheet,$ionicModal,$rootScope,$timeout) {
	...
	var closeSelectMultiple=function(){
		angular.element(document.getElementsByClassName('modal-backdrop')[0]).unbind('click',closeSelectMultiple)
		if(selectPopUpMul)selectPopUpMul.close();
	}
	...
	return {
		...
		selectMultiple:function(scope){
			
			if(selectPopUpMul)selectPopUpMul.close();
			selectPopUpMul=$ionicPopup.show({
  				//title: title, // String. The title of the popup.
				 cssClass: 'select-popup',
				templateUrl: 'screens/modal/selectMultiple.html', // String (optional). The URL of an html template to place in the popup   body.
				scope: scope, // Scope (optional). A scope to link to the popup content.
			  buttons: [{ // Array[Object] (optional). Buttons to place in the popup footer.
				text: $rootScope.idioma.General[6],
				type: 'button-default',
